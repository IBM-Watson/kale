...
On Linux, OSX and other Unix-style systems, the `alias` command can be used like this:
    ...
    alias kale="java -jar /full/path/to/kale-1.5.1-standalone.jar"

And on Windows, the `doskey` command can be used like this:

    doskey kale=java -jar C:\full\path\to\kale-1.5.1-standalone.jar $*
...
```
$ alias 'kale=java -jar /Users/ba/Downloads/kale-1.5.1-standalone.jar'
$ kale login
...
Compiling ...
...
Created .../kale/target/uberjar/kale-1.6.0-SNAPSHOT.jar
Created .../kale/target/uberjar/kale-1.6.0-SNAPSHOT-standalone.jar
```

Now you can run the tool with a simple `java` command line:

```bash
$ java -jar target/uberjar/kale-1.6.0-SNAPSHOT-standalone.jar
```
