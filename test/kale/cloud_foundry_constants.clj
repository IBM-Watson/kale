...
(defn service-entity
  ...
  {:guid guid
   ...
   :last_operation {:type "create"
                    ...
                    :updated_at nil
                    :created_at "DATE"}
   :service_plan {:guid (str "PLAN_" guid)
                  :name "standard"
                  :service {:guid (str "TYPE_" guid)
                            :label service-type
                            :provider nil
                            :version nil}}})
...
(def space-summary-response
  {:guid "SPACE_GUID"
   :name "dev"
   :apps []
   :services [(service-entity "RNR_GUID" "rnr-service" "retrieve_and_rank")
...
(defn org-entity
  ...
  {:metadata {:guid guid
              ...
              :created_at "DATE"
              :updated_at nil}
   :entity {:managers_url (str "/v2/organizations/" guid "/managers")
            :private_domains_url (str "/v2/organizations/"
                                      guid "/private_domains")
            :name name
...
(defn space-entity
  ...
  {:metadata {:guid guid
              ...
              :created_at "DATE"
              :updated_at nil}
   :entity {:apps_url (str "/v2/spaces/" guid)
            :allow_ssh true
...
(def service-keys-response
   (results-response [(service-key-entity "RNR_GUID" "retrieve_and_rank")
                      (service-key-entity "DC_GUID" "document_conversion")]))

(def orgs-response
   (results-response [(org-entity "ORG_GUID1" "org1")
                      (org-entity "ORG_GUID2" "org2")]))

(def spaces-response
   (results-response [(space-entity "SPACE_GUID1" "space1")
                      (space-entity "SPACE_GUID2" "space2")]))

(def service-type-response
   (results-response [(service-type-entity "TYPE_GUID" "service-type")]))

(def service-plan-response
   (results-response [(service-plan-entity "PLAN_GUID" "standard")]))
...
(defn service-entry
  [guid service-name service-type]
  (let [url (str "https://gateway.watsonplatform.net/" service-type "/api")]
    {(keyword service-name) {:credentials {:password "scotty"
                                           :url url
                                           :username "redshirt"}
                             :guid guid,
                             :type service-type
